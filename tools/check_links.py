...
本 fork 的公開入口互相連來連去：README、FORK、NOTICE、AGENTS 與 docs/DECISIONS.md。
文件被重新定位或改名時，這些連結會靜靜斷掉。只驗相對連結；外部網址交給人看。
不掃 commerce-common/、shopping-agent/、merchant-agent/、examples/、plugins/、scripts/、
docs/{safety,backends,deployment}.md：那些是上游產品內容，每次同步都會變。
...
# Upstream product docs: change on every sync and are not this checker's business.
SKIP_DOCS_NAMES = {
    "safety.md",
    "backends.md",
    "deployment.md",
...
def iter_documents() -> list[Path]:
    ...
    for path in (ROOT / "docs").glob("*.md"):
        if path.name not in SKIP_DOCS_NAMES:
            documents.append(path)
    github = ROOT / ".github"
    if github.is_dir():
        documents.extend(github.rglob("*.md"))
    return sorted(path for path in documents if path.is_file() and path.name not in SKIP_NAMES)
